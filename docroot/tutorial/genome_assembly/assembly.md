...
2.	This will open the report, which opens with a graph of the assembly called a Bioinformatics Application for Navigating De novo Assembly Graphs Easily, or **Bandage plot**[10], a tool for visualizing assembly graphs with connections.  
...
4.	Scrolling down will reveal a **Quast report**, which is also included in the Genome Assembly report. Quast is a quality assessment tool for evaluating and comparing genome assemblies[11], and shows statistics of the contigs generated in the assembly.  
...
3.	**Assembly Graph Plot** - A graph of the assembly is provided as part of the standard output.  Bandage (a Bioinformatics Application for Navigating De novo Assembly Graphs Easily)[10], a tool for visualizing assembly graphs with connections is used.  To see the Bandage plot, click on the row that has the .svg file and then click on the **View** icon in the vertical green bar.  The page that opens the Bandage plot, which visualizes both nodes and edges. This is particularly useful when the assembly contains many short contigs, as is often the case when assembling short reads, and empowers users to examine and assess their assembly graphs in greater detail than when viewing contigs alone. 
...
10.	The assembly pipeline also generates a file of all the contigs that did not meet the thresholds that were set when the job was submitted.  That file can be downloaded and then opened as a text file.  It can be used in other programs (like BLAST) to see if the data is valuable.
...
10.	Wick, R.R., et al., Bandage: interactive visualization of de novo genome assemblies. Bioinformatics, 2015. 31(20): p. 3350-3352.
11.	Gurevich, A., et al., QUAST: quality assessment tool for genome assemblies. Bioinformatics, 2013. 29(8): p. 1072-1075.
